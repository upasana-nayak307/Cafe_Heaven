          ...
          <div className="grid grid-cols-1 gap-3 md:hidden">
            {filteredMenuItems.map((item) => (
              <div key={item._id} className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm space-y-3">
                {/* Top Row: Title, Category, Veg/Non-Veg Type Indicator & Price */}
                <div className="flex items-start justify-between gap-2">
                  ...
                  <span className={`font-bold text-sm ${item.available ? "text-gray-900" : "text-gray-400 line-through"}`}>
                    ₹{item.price}
                  </span>
                </div>

                {/* Bottom Row: Status Badge, Toggle Switch & Action Buttons */}
